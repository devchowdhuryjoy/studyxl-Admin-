...
//     try {
//       const response = await fetch("https://stdxl.globalrouteway.com/api/admin/transactions", {
//         method: "GET",
//         headers: {
...
//     const token = getAuthToken();
//     try {
//       const response = await fetch(`https://stdxl.globalrouteway.com/api/admin/transactions/${id}`, {
//         method: "DELETE",
//         headers: {
//           "Authorization": `Bearer ${token}`,
//           "Accept": "application/json"
...
//     try {
//       const response = await fetch(`https://stdxl.globalrouteway.com/api/admin/transactions/${editData.id}`, {
//         method: "PUT",
//         headers: {
...
import React, { useState, useEffect } from "react";
import BASE_URL from "../../../Api/ApiBaseUrl"; //path adjust if needed
...
const ManageCommission = () => {
  ...
  // ✅ STATUS CONFIG
  const statusConfig = {
    fully_paid: { label: "Fully Paid", classes: "text-green-600" },
    partial: { label: "Partial", classes: "text-yellow-600" },
    unpaid: { label: "Unpaid", classes: "text-red-600" }
  };
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/admin/transactions/${editData.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const renderStatusText = (statusKey) => {
    ...
    const formatted = statusKey.toLowerCase().trim();
    const config = statusConfig[formatted];

    return (
      <span className={`font-bold ${config?.classes || "text-gray-400"}`}>
