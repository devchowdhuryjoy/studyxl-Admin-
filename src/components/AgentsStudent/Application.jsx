  ...
  const fetchApplications = async () => {
    try {
      ...
      const formatted = applicationsData.map((item) => {
        ...
        if (item.student_data?.company_name) {
          agentName = item.student_data.company_name;
        }

        agentId = item.agent_id || "N/A";
        ...
        return {
          id: item.id || item.application_id || "N/A",
          agentName: item.student_data?.company_name || "Unknown",
          agent_id: agentId,
          studentName: item.student_data?.first_name || item.student?.name || "Unknown",
          studentId: item.student_id || item.student?.id || "N/A",
          program: item.program_snapshot?.program_name || item.program?.name || "N/A",
          university: item.
            program_data
            ...
            {paginatedApplications.length === 0 ? (
              ...
              paginatedApplications.map((app) => (
                <tr key={app.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2">
                    <span
                      className={`px-2 py-1 rounded-full text-[10px] sm:text-xs ${app.status === "Accepted"
                        ? "bg-green-100 text-green-800"
                        : app.status === "Rejected"
                          ? "bg-red-100 text-red-800"
                          : app.status === "Submitted"
