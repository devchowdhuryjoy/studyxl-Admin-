...
const AgentTask = () => {
  ...
  const fetchStudentsByAgentId = async (agent_id) => {
    try {
      ...
      const transformedData = applications.map(app => ({
        ...
        // Student data from student_data object - using first_name and family_name
        first_name: app.student_data?.first_name || '',
        family_name: app.student_data?.family_name || '',
        student_email: app.student_data?.email || '',
        student_phone: app.student_data?.phone || '',
        
        // Program data from program_data object
        university_name: app.program_data?.university_name || '',
        program_name: app.program_data?.program_name || '',
        university_id: app.program_data?.university_id || app.program_data?.id,
  ...
  const fetchApplicationStatus = async (program_id, student_name) => {
    try {
      ...
      const app = response.data.applications?.find(a => {
        const fullName = `${a.student_data?.first_name || ''} ${a.student_data?.family_name || ''}`.trim();
        return fullName === student_name;
      });
  ...
  // Helper function to get student full name (combining first_name and family_name)
  const getStudentFullName = (student) => {
    return `${student.first_name || ''} ${student.family_name || ''}`.trim();
  };
            ...
            <div className="block w-full mb-5">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  // Filter applications by student full name (first_name + family_name)
                  const selectedApps = students.filter(s => {
                    const fullName = getStudentFullName(s);
                    return fullName === val;
                  });
                  
                  // Get unique universities from selected student's applications
                  const uniqueUnis = [];
                  const seenIds = new Set();
                  
                  selectedApps.forEach(app => {
                    if (!seenIds.has(app.university_id) && app.university_name) {
                      seenIds.add(app.university_id);
                      uniqueUnis.push({
                  ...
                  if (selectedApps.length > 0) {
                    setValue("student_id", selectedApps[0].student_id);
                  }
              ...
              >
                <option value="">Select Student</option>
                {students.length > 0 ? (
                  [...new Set(students.map(s => getStudentFullName(s)))].map((name, i) => (
              ...
              {students.length === 0 && (
                <p className="text-xs text-gray-500 mt-1">No students found for this agent</p>
              )}
            </div>
          )}

          {/* UNIVERSITY SELECTION */}
          {watchStudent && availableUniversities.length > 0 && (
            <div className="block w-full mb-5">
              <label className="block text-[11px] font-bold text-gray-500 uppercase mb-1.5 ml-1">University Name</label>
              ...
              >
                ...
                {availableUniversities.map((u, i) => (
                  <option key={i} value={u.university_name}>{u.university_name}</option>
                ))}
              </select>
            </div>
          )}
            ...
            <div className="block w-full mb-5">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  // Find program by matching full student name, university, and program
                  const prog = students.find(s => {
                    const fullName = getStudentFullName(s);
                    return fullName === watchStudent && 
                           s.university_name === watchUniversity && 
                           s.program_name === val;
                  });
              ...
              >
                ...
                {students
                  .filter(s => {
                    const fullName = getStudentFullName(s);
                    return fullName === watchStudent && s.university_name === watchUniversity;
                  })
                  .map((p, i) => (
                    <option key={i} value={p.program_name}>{p.program_name}</option>
                  ))}
              </select>
            </div>
          )}
            ...
            <div className="block mt-6 p-6 bg-gray-50 rounded-2xl border animate-in zoom-in">
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div className="col-span-2">
                  ...
                  <input 
                    {...register("subject")} 
                    className="block w-full bg-white p-2 border rounded-lg font-medium text-gray-700 outline-none" 
                  />
                </div>
                <div className="col-span-2">
                  <label className="text-[10px] font-bold text-gray-500 uppercase">Description</label>
                  <textarea 
                    ...
                    className="block w-full bg-white p-2 font-bold border rounded-lg text-xs text-gray-700 outline-none resize-none" 
                    rows="2" 
                  />
                </div>
                ...
                <div>
                  ...
                  <input 
                    {...register("status")} 
                    readOnly 
                    className="block w-full bg-transparent border text-gray-700 rounded-lg p-2 font-bold outline-none" 
                  />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-gray-500 uppercase">Due Date</label>
                ...
                <div>
                  ...
                  <input 
                    {...register("student_id")} 
                    readOnly 
                    className="block w-full bg-transparent border rounded-lg p-2 font-medium text-gray-700 outline-none" 
                  />
                </div>
              </div>
              ...
              <div className="block bg-white p-4 rounded-xl border-2 border-dashed border-gray-200 mt-4">
                <input 
                  ...
                  className="block w-full text-xs text-gray-400 file:mr-3 file:py-2 file:px-4 file:rounded-full file:border-0 file:bg-blue-600 file:text-white" 
                />
                {watchDocs.map((file, i) => (
                  <div key={i} className="flex justify-between bg-gray-50 p-2 rounded mt-2">
                    <span className="text-[11px] truncate">{file.name}</span>
              ...
              <button 
                type="submit" 
                className="block w-full mt-6 py-4 bg-primary text-white font-bold rounded-xl shadow-lg hover:bg-blue-700 transition-all"
              >
                Create Task & Upload
              </button>
            </div>
