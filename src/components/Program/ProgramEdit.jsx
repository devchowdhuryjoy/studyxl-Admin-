...
const handleSubmit = async (e) => {
  ...
  const monthId = intake_months?.[0]?.id || intake_id || 1;

  const updateUrl = `${BASE_URL}/admin/universities/${university_id}/${program_level_id}/${field_of_study_id}/${intake_id}/${monthId}/${program_tag_id}/${id}/update`;
  ...
  try {
    ...
    if (hasImages) {
      ...
      allowedRequirements.forEach((key) => {
        if (formData[key] !== null && formData[key] !== undefined && formData[key] !== "") {
          fd.append(`students_requirements[${key}]`, formData[key]);
        }
      });

      fd.append("students_requirements[english_exam_status][ielts][required]", formData.ielts_required ? "1" : "0");
      if (formData.ielts_reading   != null) fd.append("students_requirements[english_exam_status][ielts][reading]",   formData.ielts_reading);
      if (formData.ielts_writing   != null) fd.append("students_requirements[english_exam_status][ielts][writing]",   formData.ielts_writing);
      if (formData.ielts_listening != null) fd.append("students_requirements[english_exam_status][ielts][listening]", formData.ielts_listening);
      if (formData.ielts_speaking  != null) fd.append("students_requirements[english_exam_status][ielts][speaking]",  formData.ielts_speaking);
      if (formData.ielts_overall   != null) fd.append("students_requirements[english_exam_status][ielts][overall]",   formData.ielts_overall);

      fd.append("students_requirements[english_exam_status][toefl][required]", formData.toefl_required ? "1" : "0");
      if (formData.toefl_reading   != null) fd.append("students_requirements[english_exam_status][toefl][reading]",   formData.toefl_reading);
      if (formData.toefl_writing   != null) fd.append("students_requirements[english_exam_status][toefl][writing]",   formData.toefl_writing);
      if (formData.toefl_listening != null) fd.append("students_requirements[english_exam_status][toefl][listening]", formData.toefl_listening);
      if (formData.toefl_speaking  != null) fd.append("students_requirements[english_exam_status][toefl][speaking]",  formData.toefl_speaking);
      if (formData.toefl_overall   != null) fd.append("students_requirements[english_exam_status][toefl][overall]",   formData.toefl_overall);

      fd.append("students_requirements[english_exam_status][pte][required]", formData.pte_required ? "1" : "0");
      if (formData.pte_reading   != null) fd.append("students_requirements[english_exam_status][pte][reading]",   formData.pte_reading);
      if (formData.pte_writing   != null) fd.append("students_requirements[english_exam_status][pte][writing]",   formData.pte_writing);
      if (formData.pte_listening != null) fd.append("students_requirements[english_exam_status][pte][listening]", formData.pte_listening);
      if (formData.pte_speaking  != null) fd.append("students_requirements[english_exam_status][pte][speaking]",  formData.pte_speaking);
      if (formData.pte_overall   != null) fd.append("students_requirements[english_exam_status][pte][overall]",   formData.pte_overall);

      fd.append("students_requirements[english_exam_status][duolingo][required]", formData.duolingo_required ? "1" : "0");
      if (formData.duolingo_total != null) fd.append("students_requirements[english_exam_status][duolingo][total]", formData.duolingo_total);

      if (formData.no_exam_status != null && formData.no_exam_status !== "") {
        fd.append("students_requirements[english_exam_status][no_exam][status]", formData.no_exam_status);
      }

      if (Array.isArray(formData.images)) {
        formData.images.forEach((img, index) => {
          fd.append(`existing_images[${index}]`, img);
        });
      }
      ...
      response = await axios.post(updateUrl, fd, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "multipart/form-data",
        },
      ...
      payload.students_requirements = {};

      allowedRequirements.forEach((key) => {
        if (formData[key] !== null && formData[key] !== undefined && formData[key] !== "") {
          payload.students_requirements[key] = formData[key];
        }
      });

      payload.students_requirements.english_exam_status = {
        ielts: {
          required:  formData.ielts_required ?? false,
          ...
          overall:   formData.ielts_overall,
        },
        toefl: {
          required:  formData.toefl_required ?? false,
          reading:   formData.toefl_reading,
          writing:   formData.toefl_writing,
          listening: formData.toefl_listening,
          speaking:  formData.toefl_speaking,
          overall:   formData.toefl_overall,
        },
        pte: {
          required:  formData.pte_required ?? false,
          reading:   formData.pte_reading,
          writing:   formData.pte_writing,
          listening: formData.pte_listening,
          speaking:  formData.pte_speaking,
          overall:   formData.pte_overall,
        },
        duolingo: {
          required: formData.duolingo_required ?? false,
          total:    formData.duolingo_total,
        },
        no_exam: {
    ...
    if (response.data) {
      Swal.fire({
        ...
        timer: 2000,
        showConfirmButton: false,
      }).then(() => navigate("/dashboard/programshow"));
    }
